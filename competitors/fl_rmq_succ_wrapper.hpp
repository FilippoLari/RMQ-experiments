...
template<typename K, typename Range, typename Pos,
 typename Floating = float, size_t ExcSamples = 2048,
 size_t DataSamples = 0, size_t Epsilon = 4096>
class SuccinctFLRMQWrapper : public EncodingFLRMQ<K, Range, Pos, Floating, ExcSamples, DataSamples, Epsilon> {

    size_t n;

public:
    
    SuccinctFLRMQWrapper(const std::vector<K> &data) : n(data.size()), EncodingFLRMQ<K, Range, Pos, Floating, ExcSamples, DataSamples, Epsilon>(data) {}

    inline size_t query(const size_t i, const size_t j) const {
        return EncodingFLRMQ<K, Range, Pos, Floating, ExcSamples, DataSamples, Epsilon>::query(i, j);
    }
